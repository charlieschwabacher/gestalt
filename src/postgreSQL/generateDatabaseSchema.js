...
import type {Document, Node, ObjectTypeDefinition, FieldDefinition, Directive,
  Type} from 'graphql/language/ast';
import type {DatabaseSchema, Table, Index, Column, ColumnType} from './types';
...
export default function generateDatabaseSchema(ast: Document): DatabaseSchema {
  ...
  ast.definitions.forEach(definition => {
    if (isDatabaseType(definition)) {
      tables.push(tableFromObjectTypeDefinition(definition));
      indices.push(idIndexFromObjectTypeDefinition(definition));
      edges.push(...edgesFromObjectTypeDefinition(definition));
    }
  });

  // create join tables, indices, and foreign key columns for object
  // edges
  edges.forEach(edge => {
...
function isDatabaseType(definition: Node): boolean {
...
function isDatabaseField(definition: FieldDefinition): boolean {
...
function tableFromObjectTypeDefinition(
...
function columnFromFieldDefintion(definition: FieldDefinition): Column {
  return {
    name: snake(definition.name.value),
    type: columnTypeFromGraphQLType(definition.type),
    primaryKey: definition.name.value === 'id',
    nonNull: definition.type.kind === 'NonNullType',
  };
}

function columnTypeFromGraphQLType(type: Type): ColumnType {
...
function idIndexFromObjectTypeDefinition(
  definition: ObjectTypeDefinition,
): Index {
  return {
...
function edgesFromObjectTypeDefinition(
  definition: ObjectTypeDefinition,
): [Edge] {
  ...
  definition.fields.forEach(field => {
    if (field.directives) {
      ...
      if (edgeDirective) {
        edges.push(edgeFromDirective(fromType, 'Post', edgeDirective));
      }
    }
  });
...
function edgeFromDirective(
  ...
  return edgeFromPathString(fromType, toType, pathArgument.value.value);
}
...
export function edgeFromPathString(
  ...
  path.push(edgeSegmentFromParts(fromType, left, label, right, finalType));
...
export function edgeSegmentFromParts(
  ...
  return Object.assign({fromType, toType, label}, arrow);
}
